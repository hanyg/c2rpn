...
char ops[] = "HALT,ENT ,LEV ,JMP ,JMPI,JSR ,JSRA,LEA ,LEAG,CYC ,MCPY,MCMP,MCHR,MSET,"	// system
  "LL  ,LLS ,LLH ,LLC ,LLB ,LLD ,LLF ,LG  ,LGS ,LGH ,LGC ,LGB ,LGD ,LGF ,"	// load a
  "LX  ,LXS ,LXH ,LXC ,LXB ,LXD ,LXF ,LI  ,LHI ,LIF ," "LBL ,LBLS,LBLH,LBLC,LBLB,LBLD,LBLF,LBG ,LBGS,LBGH,LBGC,LBGB,LBGD,LBGF,"	// load b
  "LBX ,LBXS,LBXH,LBXC,LBXB,LBXD,LBXF,LBI ,LBHI,LBIF,LBA ,LBAD," "SL  ,SLH ,SLB ,SLD ,SLF ,SG  ,SGH ,SGB ,SGD ,SGF ,"	// store
  "SX  ,SXH ,SXB ,SXD ,SXF ," "ADDF,SUBF,MULF,DIVF,"	// arithmetic
  "ADD ,ADDI,ADDL,SUB ,SUBI,SUBL,MUL ,MULI,MULL,DIV ,DIVI,DIVL," "DVU ,DVUI,DVUL,MOD ,MODI,MODL,MDU ,MDUI,MDUL,AND ,ANDI,ANDL," "OR  ,ORI ,ORL ,XOR ,XORI,XORL,SHL ,SHLI,SHLL,SHR ,SHRI,SHRL," "SRU ,SRUI,SRUL,EQ  ,EQF ,NE  ,NEF ,LT  ,LTU ,LTF ,GE  ,GEU ,GEF ,"	// logical
  ...
  "IVEC,PDIR,SPAG,TIME,LVAD,TRAP,LUSP,SUSP,LCL ,LCA ,PSHC,POPC,MSIZ," "PSHG,POPG,NET1,NET2,NET3,NET4,NET5,NET6,NET7,NET8,NET9," "POW ,ATN2,FABS,ATAN,LOG ,LOGT,EXP ,FLOR,CEIL,HYPO,SIN ,COS ,TAN ,ASIN,"	// math
  "ACOS,SINH,COSH,TANH,SQRT,FMOD," "IDLE,";
...
char *
mapfile (char *name, int size)	// XXX replace with mmap
{
  int f;
  char *p;
...
again:
  for (;;)
	...
	case '#':
	  if (mlevel)
	    {
	      err ("bad preprocessor directive");
	      exit (-1);
	    }
		  ...
		  if (id->macro && mlevel != -1)
		    {
		      if (mlevel == MSTACK_SZ)
			{
			  err ("exceeded macro recursion level");
			  exit (-1);
			}
		      mstack[mlevel++] = pos;
		      pos = id->macro;
...
int
imm (void)			/// XXX move these back down once I validate prototypes working for double immf()
{
  N b = e;
  int c;
  expr (Cond);
...
int
tsize (uint t)			// XXX return unsigned? or error checking on size
{
  array_t *a;
  struct_t *s;
  switch (t & TMASK)
...
int
tinc (uint t)			// XXX return unsigned?
{
  if (t & PMASK)
    return tsize (t - PTR);
  else if (t & ARRAY)
    return tsize (((array_t *) (va + (t >> TSHIFT)))->type);	// XXX need to test this!
  else
    return 1;
}
      ...
      for (pt = p = 0; tk != ')'; p++)
	{
	  ...
	  if (n)
		...
		*v = (ident_t *) ploc;	// hack if function name same as parameter
	      ...
	      ploc++;
	      if ((d & TMASK) == ARRAY)
		d = ((array_t *) (va + (d >> TSHIFT)))->type + PTR;	// convert array to pointer
	      n->local = 1;
	      n->class = Auto;
	      n->type = d;
	      n->val = p * 8 + 8;
	  ...
	  if (tk == Comma)
	    next ();		// XXX desparately need to flag an error if not a comma or close paren!
	  if (tk == Dots)
	    {
	      next ();
	      if (tk != ')')
...
//        if (bc != Static || sc != Static) err("bad nested function declaration");
		...
		err ("duplicate definition");
	      ...
	      v->type = t;
	      if (sc != Typedef)
		{		// XXX typedefs local to functions?
		  if ((t & TMASK) == ARRAY)
		      ...
		      if (tk == Assign)
			{
			  ...
			  if (tk == '"')
			      ...
			      while (tk != '}')
				{
			      ...
			      next ();
			      if (size)
				data = hglo + size * align;	// XXX need to zero fill if size > initialized part  XXX but may default since using sbrk vs malloc?
...
void
nodc (int n, N a, N b)		// commutative
{
  (e -= 4)->i = n;
  if (a->i < b->i)
    {
...
void
mul (N b)			// XXX does this handle unsigned correctly?
{
  if (b->i == Num)
    {
      if (e->i == Num)
...
void
add (N b)			// XXX make sure to optimize (a + 9 + 2) -> (a + 11)    and    (a + 9 - 2) -> (a + 7)
{
  if (b->i == Num)
    {
      if (e->i == Num || e->i == Lea || e->i == Leag)
	{
	  e[2].u += b[2].u;
	  return;
	}			// XXX  <<>> check
...
assign (int n, N b)
{
  ...
  e[1].n = b;
  switch (ty)
    {				// post-cast usually removed by trim()
    case CHAR:
      (e -= 2)->i = Cic;
      return;
    case UCHAR:
...
void
trim (void)			// trim dead code from expression statements (just the common cases)
{
  if (e->i >= Cic && e->i <= Cus)
    e += 2;			// remove conversion after assignment
  if (e->i == Add && e[2].n->i == Num)
    e = e[1].n;			// convert x++ into ++x
}
...
lbf (N b)
{
  ...
  switch (b->i)
    {
    case Auto:
      eml (LBL + lmod (b[1].i), b[2].i);
      return;
    case Static:
      emg (LBG + lmod (b[1].i), b[2].i);
      return;
    case Numf:
      d = *(double *) (b + 2);
      if (((int) (d * 256) << 8 >> 8) / 256 == d)
	emi (LBIF, d * 256);
      else
	{
	  data = (data + 7) & -8;
...
opaf (N a, int o, int comm)
{
  ...
  switch (a->i)
	...
	{
	  rv (b);
	  eml (LBL + lmod (a[1].i), a[2].i);
	}
      else
	{
	  lbf (b);
	  eml (LL + lmod (a[1].i), a[2].i);
	  if (!t)
	    em (a[1].i < UINT ? CID : CUD);
	}
      em (o);
      if (!t)
	em (a[1].i < UINT ? CDI : CDU);
	...
	{
	  rv (b);
	  emg (LBG + lmod (a[1].i), a[2].i);
	}
      else
	{
	  lbf (b);
	  emg (LG + lmod (a[1].i), a[2].i);
	  if (!t)
	    em (a[1].i < UINT ? CID : CUD);
	}
      em (o);
...
opa (N a, int o, int comm)
{
  ...
  switch (a->i)
    {
    case Auto:
      if (b->i == Num && b[2].i << 8 >> 8 == b[2].i)
	{
	  eml (LL + lmod (a[1].i), a[2].i);
	  emi (o + OPI, b[2].i);
	}			// loc op= num
      else if (b->i == Auto && !lmod (b[1].i))
	{
	  eml (LL + lmod (a[1].i), a[2].i);
	  eml (o + OPL, b[2].i);
	}			// loc op= locint
...
testnot (N a, int t)
{
  ...
  switch (a->i)
    ...
    case Lor:
      ...
      return t;
    case Lan:
      return testnot (a + 2, testnot (a[1].n, t));
    case Not:
      return test (a + 2, t);
    case Notf:
      rv (a + 2);
  ...
  switch (a->i)
	...
	case Ptr:
	  switch (a[2].i)
	    ...
	    case Auto:
	      rv (b);
	      eml (LBL + lmod (a[3].i), a[4].i);
	      break;		// *loc = expr // XXX all good? in the presense of multiple casts?
	    case Static:
	      rv (b);
	      emg (LBG + lmod (a[3].i), a[4].i);
	      break;		// *glo = expr
	    default:
	      rv (a + 2);
	      switch (b->i)
    ...
    case Fcall:
      ...
      for (t = 0; b; t += 8)
	{
	  if (b[1].i == DOUBLE || b[1].i == FLOAT)
	    {
	      rv (b + 2);
	      loc -= 8;
	      em (PSHF);
	    }
	  else if (b[2].i == Num && b[4].i << 8 >> 8 == b[4].i)
	    {
	      loc -= 8;
	      emi (PSHI, b[4].i);
	    }
...
stmt (void)
{
  ...
  switch (tk)
    ...
    case Switch:
      ...
      patch (a, ip);
      if (es == e)
	{			//err("no case in switch statement");   XXX
	  if (def)
	    emj (JMP, def);
	}
      else
	{			// XXX lots of possible signed/unsigned under/overflow issues in this block
	  cmin = cmax = (et = es - 2)->i;
	  while (et > e)
	    {
	      et -= 2;
	  ...
	  et = es;
	  if ((ulong) (cmax - cmin) <= (es - e) * 8)
	    {			// jump table
	      if (cmin > 0 && cmax <= (es - e) * 4)
		cmin = 0;
	      else if (cmin)
		{
		  opi (SUB, cmin);
		...
		{
		  et -= 2;
		  ((int *) (gs + data))[et->i - cmin] = et[1].i - (ts + ip);
		}
	      data += cmax * 4;
	    }
	  else
...
{
  int i, text, *patchdata, *patchbss;
  long amain, sbrk_start;
  ident_t *tmain;
  char *outfile;
  struct
  {
  ...
  while (--argc && *file == '-')
	...
	default:
	usage:dprintf (2, "usage: %s [-v] [-s] [-Ipath] [-o exefile] file ...\n",
		   cmd);
	  return -1;
	}
      file = *++argv;
  ...
  if (!errs && !debug)
	...
	{
	  pdata--;
	  *(int *) (ts + *pdata) += (ip - *pdata - 4) << 8;
	}
      while (pbss != patchbss)
	{
	  pbss--;
	  *(int *) (ts + *pbss) += (ip + data - *pbss - 4) << 8;
	}
      if (outfile)
	{
